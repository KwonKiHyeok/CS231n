...
def adam(w, dw, config=None):
    ...
    # Step 1: Update time step first (very important for matching reference)
    config["t"] += 1
    t = config["t"]

    # Step 2: Update biased first and second moment estimates
    config["m"] = config["beta1"] * config["m"] + (1- config["beta1"]) * dw # Momentum
    config["v"] = config["beta2"] * config["v"] + (1 - config["beta2"]) * dw * dw # RMSProp & AdaGrad
    
    # Step 3: Compute bias-corrected moment estimates
    first_unbias = config["m"] / (1 - config["beta1"] ** t) 
    second_unbias = config["v"] / (1 - config["beta2"] ** t)
